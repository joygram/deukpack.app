...
**이름·옵션·생성 API**를 빠르게 찾을 때 쓰는 문서입니다. 제품이 **무엇을 하는지**는 [제품군 개요](../products/index.ko.md), **따라 하기**는 [튜토리얼](../tutorial/index.ko.md)을 보세요.
...
| 분류 | 무엇을 다루나 | 문서 |
|------|----------------|------|
| **기본 구성** | IDL 키워드(`record`·`message`·`table`·`entity`), CLI·include, 라이브러리 파싱 | [기본 구성](fundamentals.ko.md) |
| **테이블** | `table`·메타 테이블, 행 레코드, `MetaTableRegistry`, `GetSchema` | [테이블](tables.ko.md) |
| **메시지·와이어** | `message`, `ProtocolRegistry`, `Write`/`Read`, 통합 Write(서브셋·오버라이드) | [메시지·와이어](messages.ko.md) |
| **와이어·프로토콜 계열** | Thrift 호환(`tbinary`·`tcompact`·`tjson`) vs 득팩 전용(`pack`·`json`·`yaml`), JS+스키마, npm `serialize` 기본값 | [와이어 프로토콜 계열](wire-protocols.ko.md) |
