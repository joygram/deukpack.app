...
| 5 | [**WriteWithOverrides·WriteFields·extends**](write-with-overrides.md) | 교체(수신자별 필드 값 다르게), 선택(원하는 필드만 직렬화), 상속(공통 필드 한 번 정의). 기능 비교표 포함. |
...
| 문서 | 대상 | 내용 |
|------|------|------|
| [**C# 가이드**](csharp-guide.md) | .NET·Unity 개발자 | 생성 코드, 읽기/쓰기, **WriteWithOverrides**, GetSchema·ProtocolRegistry |
...
- **팬아웃·수신자별 필드**: [WriteWithOverrides](write-with-overrides.md) → [API·타입 참조](../reference/api.md)
