...
Output goes under `gen/csharp/` (or your specified folder) as `.cs` files.  
...
Generated types serialize via **IDeukPackReader** / **IDeukPackWriter** (or protocol-specific Reader/Writer). Select the protocol matching your `--protocol binary` (or compact, json) at generation time.
...
- **Starter kits**: See the Unity and Web App entries in [Starter kits](../starter-kits.md) for sample paths and setup guidance.
