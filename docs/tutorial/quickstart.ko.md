...
- **C#·Unity**: 생성 코드를 솔루션에 포함하고, [프로토콜](../products/protocol.md) 문서를 참고해 직렬화·메시지 처리를 연결합니다. npm 패키지의 `dist/csharp` 런타임 또는 [득팩 키트 라인업](../starter-kits.md)을 활용할 수 있습니다.
...
*CLI 옵션 전체는 터미널에서 `npx deukpack --help` 또는 [API·타입 참조](../reference/api.md)를 확인하세요. 호환 vs 득팩 전용 표는 [와이어 프로토콜 계열](../reference/wire-protocols.md).*
